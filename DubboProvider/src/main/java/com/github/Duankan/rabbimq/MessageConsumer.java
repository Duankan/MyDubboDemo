 ...
 * @author duankang
 * @date 2019-06-30
 * @class MessageConsumer 消费者基础类
 * @desc 每天学习一点（明年月薪12k）
 */
public class MessageConsumer implements Consumer {

    protected String consumerTag;//消费者标签，注册成功时由rabbitmq服务器自动生成
    ...
    public void basicConsume() {
        try {
             ...
             * @param queue 队列名称
             * @param autoAck 是否自动应答。false表示consumer在成功消费过后必须要手动回复一下服务器，
...
//        System.out.println("消费者：" + consumerTag + ",注册成功！");
    ...
    /**
     * @param consumerTag
     * @func 当消费者因为其他原因被动取消注册时调用，比如queue被删除了。
     */
